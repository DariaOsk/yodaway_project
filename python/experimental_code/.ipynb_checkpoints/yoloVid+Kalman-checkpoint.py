...
def main():
    ...
    # init deque for visual following of centroids
    qu = deque(maxlen=dequelen)
    ...
    while True: 
        ...
        # # load centroids into dequeue
        # qu.append(centroids)
        
        # # loop over set of tracked points
        # for i in range(1, len(qu)):
        #     if qu[i-1] is None or qu[i] is None:
        #         continue
        #     for j in range(1, len(centroids)):
        #         print((int(qu[i-1][j][0]),int(qu[i-1][j][1])))
        #         #compute thickness of line (the more recent the thicker)
        #         thickness = int(np.sqrt(dequelen/float(i+1))*2.5)
        #         cv2.line(frame, (int(qu[i-1][j][0]),int(qu[i-1][j][1])), (int(qu[i][j][0]),int(qu[i][j][1])), (0,0,255), thickness)
